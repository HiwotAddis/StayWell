...
const RoomDetails = () => {
  ...
  return room && (
    <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
        ...
        <div className='flex flex-col lg:flex-row gap-6 mt-6'>
            ...
            <div className='grid grid-cols-2 lg:w-1/2 w-full gap-4'>
                {room.images.length >1 && room.images.map((image, index) => (
                    <img onClick={() => setMainImage(image)} key={index} src={image} alt={'Room Image'} className={`w-full rounded-xl shadow-md cursor-pointer object-cover ${mainImage === image && 'outline-3 outline-orange-500'}`} />
                ))}
            </div>
        </div>
        {/*Room Highlights*/}
        ...
        <form className='flex flex-col md:flex-row items-start md:items-center mt-16 justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto max-w-6xl'>
        ...
        <div className='max-w-3xl border-y border-gray-300 my-15 py-10 text-gray-500'>
            <p>
                ...
                price quoted is for two guests, at the guest slot please mark the number of guests to get the exact price for groups.  
            </p>
        </div>
        {/*Hosted By*/}
